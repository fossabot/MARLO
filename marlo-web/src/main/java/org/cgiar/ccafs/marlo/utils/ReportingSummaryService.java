...
/**
 * A service whose logic can be reused rather than injecting struts2 actions within other
 * struts2 actions. :-(
 * 
 * @author GrantL
 */
...
public class ReportingSummaryService {
  ...
  private final Logger LOG = LoggerFactory.getLogger(ReportingSummaryService.class);
  ...
  public ReportingSummaryService(ResourceManager resourceManager, CrpProgramManager programManager,
    GenderTypeManager genderTypeManager, RepositoryChannelManager repositoryChannelManager,
    LocElementManager locElementManager, CrossCuttingScoringManager crossCuttingScoringManager,
    LocalizedTextProvider localizedTextProvider, ProjectBudgetManager projectBudgetManager, FAIRService fairService,
    IpElementManager ipElementManager, InstitutionManager institutionManager, APConfig apConfig) {
   ...
   */
  private MasterReport addColumnParameters(MasterReport masterReport, ReportingSummaryContext context) {
    ...
    if (context.getHasGender()) {
      columnBudgetPartner++;
    } else {
      exludeIndex.add(0);
    }
    if (context.getHasW1W2Co()) {
      columnBudgetPartner++;
    } else {
      exludeIndex.add(2);
    }
   ...
   */
  private MasterReport addi8nParameters(MasterReport masterReport, ReportingSummaryContext context) {
    Boolean hasActivities = false;
    try {
      hasActivities = context.isActivitiesSpecifityEnabled();
    } catch (Exception e) {
    ...
    if (context.getProject().getProjectInfo().getAdministrative() != null
      && context.getProject().getProjectInfo().getAdministrative() == true) {
    ...
    masterReport.getParameterValues().put("i8nProjectOutcomesRAnnualProgress", this
      .getText("projectOutcomes.annualProgress.readText", new String[] {String.valueOf(context.getSelectedYear())}));
    masterReport.getParameterValues().put("i8nProjectOutcomesRAnnualProgressCurrent",
      this.getText("projectOutcomes.annualProgressCurrentReporting.readText",
        new String[] {String.valueOf(context.getSelectedYear())}));
  ...
  private String calculateAcumulativeTarget(int yearCalculate, IpProjectIndicator id, ReportingSummaryContext context) {
    int acumulative = 0;
    try {
      for (IpProjectIndicator indicators : context.getProject().getProjectIndicators()) {
        if (indicators != null) {
  ...
  private boolean containsOutput(long outputID, long outcomeID, ReportingSummaryContext context) {
    if (context.getProject().getMogs() != null) {
      for (IpElement output : context.getProject().getMogs()) {
  ...
  private void fillSubreport(SubReport subReport, String query, List<Object> args, ReportingSummaryContext context) {
    ...
    switch (query) {
      case "description":
        if (args.get(0) instanceof ProjectPartner) {
          model = this.getDescTableModel((ProjectPartner) args.get(0), (Boolean) args.get(1), context);
        } else {
          model = this.getDescTableModel(new ProjectPartner(), (Boolean) args.get(1), context);
        }
        break;
      ...
      case "description_regions":
        model = this.getRLTableModel((List<CrpProgram>) args.get(0), context);
        break;
      case "description_coas":
        model = this.getDescCoAsTableModel(context);
        break;
      case "partners_count":
        model = this.getPartnersTableModel(context);
        break;
      ...
      case "partners_others_ins":
        if (args.get(0) instanceof ProjectPartner) {
          model = this.getPartnersOtherTableModel((ProjectPartner) args.get(0), context);
        } else {
          model = this.getPartnersOtherTableModel(new ProjectPartner(), context);
        }
        break;
      case "partner_lessons":
        model = this.getPartnersLessonsTableModel(context);
        break;
      case "locations":
        model = this.getLocationsTableModel(context);
        break;
      case "outcomes_list":
        model = this.getOutcomesTableModel(context);
        break;
      case "expected_studies":
        model = this.getExpectedStudiesTableModel(context);
        break;
      case "deliverables_list":
        model = this.getDeliverablesTableModel(context);
        break;
      case "activities_list":
        model = this.getActivitiesTableModel(context);
        break;
      case "activities_reporting_list":
        model = this.getActivitiesReportingTableModel(context);
        break;
      case "budget_summary":
        model = this.getBudgetSummaryTableModel(context);
        break;
      case "budgets_by_partners_list":
        model = this.getBudgetsbyPartnersTableModel(context);
        break;
      case "budgets_by_coas_list":
        model = this.getBudgetsbyCoasTableModel(context);
        break;
      // Especific for reporting
      case "project_outcomes":
        model = this.getProjectOutcomesTableModel(context);
        break;
      case "other_outcomes":
        model = this.getProjectOtherOutcomesTableModel(context);
        break;
      case "ccafs_outcomes":
        model = this.getccafsOutcomesTableModel(context);
        break;
      case "other_contributions":
        model = this.getOtherContributionsTableModel(context);
        break;
      case "other_contributions_detail":
        model = this.getOtherContributionsDetailTableModel(context);
        break;
      case "other_contributions_crps":
        model = this.getOtherContributionsCrpsTableModel(context);
        break;
      case "case_studies":
        model = this.getCaseStudiesTableModel(context);
        break;
      case "overview_by_mogs":
        model = this.getOverviewByMogsTableModel(context);
        break;
      case "deliverables_list_reporting":
        model = this.getDeliverablesReportingTableModel(context);
        break;
      case "project_highlight":
        model = this.getProjectHighlightReportingTableModel(context);
        break;
      case "leverages":
        model = this.getLeveragesTableModel(context);
        break;
    }
  ...
  private TypedTableModel getActivitiesReportingTableModel(ReportingSummaryContext context) {
    ...
    if (!context.getProject().getActivities().isEmpty()) {
      for (Activity activity : context.getProject().getActivities().stream()
        ...
        .filter(a -> a.isActive() && a.getActivityStatus() != null
          ...
          && a.getPhase().equals(context.getSelectedPhase()))
        ...
        if (cal.get(Calendar.YEAR) >= context.getSelectedYear()
          || cal2.get(Calendar.YEAR) >= context.getSelectedYear()) {
  ...
  private TypedTableModel getActivitiesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      new String[] {"activity_id", "title", "description", "start_date", "end_date", "institution", "activity_leader",
        "status", "deliverables"},
      new Class[] {Long.class, String.class, String.class, String.class, String.class, String.class, String.class,
    ...
    if (!context.getProject().getActivities().isEmpty()) {
      for (Activity activity : context.getProject().getActivities().stream()
        ...
        List<DeliverableActivity> deliverableActivityList = activity.getDeliverableActivities().stream()
          .filter(da -> da.isActive() && da.getPhase() != null && da.getPhase().equals(context.getSelectedPhase()))
          .collect(Collectors.toList());
        if (deliverableActivityList != null && !deliverableActivityList.isEmpty()) {
          for (DeliverableActivity deliverableActivity : deliverableActivityList) {
            ...
            if (deliverableActivity.getDeliverable().getDeliverableInfo(context.getSelectedPhase())
              .getTitle() != null) {
              deliverableTitle =
                deliverableActivity.getDeliverable().getDeliverableInfo(context.getSelectedPhase()).getTitle();
            } else {
  ...
  private ProjectBudgetsCluserActvity getBudgetbyCoa(Long activitiyId, int year, long type,
    ReportingSummaryContext context) {
    for (ProjectBudgetsCluserActvity pb : context.getProject().getProjectBudgetsCluserActvities().stream()
      .filter(pb -> pb.isActive() && pb.getYear() == year && pb.getCrpClusterOfActivity() != null
        ...
        && pb.getPhase() != null && pb.getPhase().equals(context.getSelectedPhase()))
      .collect(Collectors.toList())) {
      return pb;
    }
    return null;
  ...
  private TypedTableModel getBudgetsbyCoasTableModel(ReportingSummaryContext context) {
    ...
    coAs = context.getProject().getProjectClusterActivities().stream()
      .filter(c -> c.isActive() && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
      .collect(Collectors.toList());
    /*     */
    ...
    if (coAs.size() == 1 && context.getHasW1W2Co()) {
      hasW1W2CoTemp = true;
      // W1W2 no including co
      totalW1w2 = this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 3, context);
      // W1W2 including co
      totalW1w2Co = this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 2, context);

    } else {
      totalW1w2 = this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 1, context);
    }

    totalW3 = this.getTotalYear(context.getSelectedYear(), 2, context.getProject(), 1, context);
    totalBilateral = this.getTotalYear(context.getSelectedYear(), 3, context.getProject(), 1, context);
    totalCenter = this.getTotalYear(context.getSelectedYear(), 4, context.getProject(), 1, context);

    // get total gender per year
    for (ProjectPartner pp : context.getProject().getProjectPartners().stream()
      .filter(pp -> pp.isActive() && pp.getPhase() != null && pp.getPhase().equals(context.getSelectedPhase()))
      .collect(Collectors.toList())) {
      if (this.isPPA(pp.getInstitution(), context)) {
        if (coAs.size() == 1 && context.getHasW1W2Co()) {
          totalW1w2CoGender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 1,
            context.getProject(), 2, context);
          totalW1w2Gender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 1,
            context.getProject(), 3, context);
        } else {
          totalW1w2Gender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 1,
            context.getProject(), 1, context);
        }
        totalW3Gender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 2,
          context.getProject(), 1, context);
        totalBilateralGender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 3,
          context.getProject(), 1, context);
        totalCenterGender += this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(), 4,
          context.getProject(), 1, context);
      }
    }
    /**/
    if (coAs.size() == 1) {
      ...
      for (ProjectBudget pb : context.getProject()
        .getProjectBudgets().stream().filter(pb -> pb.isActive() && pb.getYear() == context.getSelectedYear()
          && pb.getBudgetType() != null && pb.getPhase() != null && pb.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {

        if (pb.getBudgetType().getId() == 1 && pb.getFundingSource() != null
          && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2() != null
          && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2()) {
          w1w2CoFinancing = "100";
      ...
      model.addRow(new Object[] {description, context.getSelectedYear(), w1w2, w3, bilateral, center, w1w2GenderPer,
      ...
      for (ProjectClusterActivity clusterActivity : coAs) {
        ...
        ProjectBudgetsCluserActvity w1w2pb =
          this.getBudgetbyCoa(clusterActivity.getCrpClusterOfActivity().getId(), context.getSelectedYear(), 1, context);
        if (w1w2pb != null) {
        ...
        ProjectBudgetsCluserActvity w3pb =
          this.getBudgetbyCoa(clusterActivity.getCrpClusterOfActivity().getId(), context.getSelectedYear(), 2, context);
        ProjectBudgetsCluserActvity bilateralpb =
          this.getBudgetbyCoa(clusterActivity.getCrpClusterOfActivity().getId(), context.getSelectedYear(), 3, context);
        ProjectBudgetsCluserActvity centerpb =
          this.getBudgetbyCoa(clusterActivity.getCrpClusterOfActivity().getId(), context.getSelectedYear(), 4, context);
        ...
        model.addRow(new Object[] {description, context.getSelectedYear(), w1w2Percentage, w3Percentage,
  ...
  private TypedTableModel getBudgetsbyPartnersTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
    ...
    for (ProjectPartner pp : context.getProject().getProjectPartners().stream()
      ...
      if (this.isPPA(pp.getInstitution(), context)) {
        ...
        if (context.getHasW1W2Co()) {
          w1w2Budget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 3, context)));
          w1w2CoBudget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 2, context)));

          w1w2Gender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(), context.getSelectedYear(),
            1, context.getProject().getId(), 3, context));
          w1w2CoGender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 2, context));

          w1w2GAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(),
            1, context.getProject().getId(), 3, context));
          w1w2CoGAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(),
            1, context.getProject().getId(), 2, context));

          // increment partner total
          partnerTotald += Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 3, context));
          partnerTotald += Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 2, context));
        } else {
          w1w2Budget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
            context.getSelectedYear(), 1, context.getProject().getId(), 1, context)));
          w1w2Gender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(), context.getSelectedYear(),
            1, context.getProject().getId(), 1, context));
          w1w2GAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(), context.getSelectedYear(),
            1, context.getProject().getId(), 1, context));
        ...
        String w3Budget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
          context.getSelectedYear(), 2, context.getProject().getId(), 1, context)));
        String bilateralBudget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
          context.getSelectedYear(), 3, context.getProject().getId(), 1, context)));
        String centerBudget = myFormatter.format(Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(),
          context.getSelectedYear(), 4, context.getProject().getId(), 1, context)));


        String w3Gender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(),
          context.getSelectedYear(), 2, context.getProject().getId(), 1, context));
        String bilateralGender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(),
          context.getSelectedYear(), 3, context.getProject().getId(), 1, context));
        String centerGender = myFormatter.format(this.getTotalGenderPer(pp.getInstitution().getId(),
          context.getSelectedYear(), 4, context.getProject().getId(), 1, context));


        String w3GAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(),
          context.getSelectedYear(), 2, context.getProject().getId(), 1, context));
        String bilateralGAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(),
          context.getSelectedYear(), 3, context.getProject().getId(), 1, context));
        String centerGAmount = myFormatter.format(this.getTotalGender(pp.getInstitution().getId(),
          context.getSelectedYear(), 4, context.getProject().getId(), 1, context));

        // increment partner total
        partnerTotald += Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(), context.getSelectedYear(),
          2, context.getProject().getId(), 1, context));
        partnerTotald += Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(), context.getSelectedYear(),
          3, context.getProject().getId(), 1, context));
        partnerTotald += Double.parseDouble(this.getTotalAmount(pp.getInstitution().getId(), context.getSelectedYear(),
          4, context.getProject().getId(), 1, context));

        // set partner total
        partnerTotal = "$" + myFormatter.format(partnerTotald);

        model.addRow(new Object[] {context.getSelectedYear(), pp.getInstitution().getComposedName(), w1w2Budget,
          w3Budget, bilateralBudget, centerBudget, pp.getInstitution().getId(), context.getProject().getId(),
          w1w2Gender, w3Gender, bilateralGender, centerGender, w1w2GAmount, w3GAmount, bilateralGAmount, centerGAmount,
          w1w2CoBudget, w1w2CoGender, w1w2CoGAmount, partnerTotal});
      }
    }
    return model;
  }

  private TypedTableModel getBudgetSummaryTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
    ...
    if (context.getHasW1W2Co()) {
      w1w2 = myFormatter.format(this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 3, context));
      w1w2CoFinancing =
        myFormatter.format(this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 2, context));
      // increment Budget Total with w1w2 cofinancing
      grand_totald += this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 3, context)
        + this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 2, context);
    } else {
      w1w2 = myFormatter.format(this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 1, context));
      // increment Budget Total with w1w2
      grand_totald += this.getTotalYear(context.getSelectedYear(), 1, context.getProject(), 1, context);
    }
    w3 = myFormatter.format(this.getTotalYear(context.getSelectedYear(), 2, context.getProject(), 1, context));
    bilateral = myFormatter.format(this.getTotalYear(context.getSelectedYear(), 3, context.getProject(), 1, context));
    centerfunds = myFormatter.format(this.getTotalYear(context.getSelectedYear(), 4, context.getProject(), 1, context));
    // increment Budget Total with w3,bilateral and centerfunds
    grand_totald += this.getTotalYear(context.getSelectedYear(), 2, context.getProject(), 1, context)
      + this.getTotalYear(context.getSelectedYear(), 3, context.getProject(), 1, context)
      + this.getTotalYear(context.getSelectedYear(), 4, context.getProject(), 1, context);
    grand_total = "$" + myFormatter.format(grand_totald);

    model
      .addRow(new Object[] {context.getSelectedYear(), w1w2, w3, bilateral, centerfunds, w1w2CoFinancing, grand_total});
    return model;
  }

  private TypedTableModel getCaseStudiesTableModel(ReportingSummaryContext context) {
    ...
    for (CaseStudyProject caseStudyProject : context.getProject()
      .getCaseStudyProjects().stream().filter(csp -> csp.isActive() && csp.getCaseStudy() != null
        && csp.getCaseStudy().getYear() != null && csp.getCaseStudy().getYear() >= context.getSelectedYear())
      .collect(Collectors.toList())) {
      ...
      if (caseStudy.getFile() != null) {
        anex = (this.getCaseStudyUrl(shared, context) + caseStudy.getFile().getFileName()).replace(" ", "%20");
      }
  ...
  private String getCaseStudyUrl(String project, ReportingSummaryContext context) {
    return apConfig.getDownloadURL() + "/" + this.getCaseStudyUrlPath(project, context).replace('\\', '/');
  }

  private String getCaseStudyUrlPath(String project, ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + project + File.separator
      + "caseStudy" + File.separator;
  }

  private TypedTableModel getccafsOutcomesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      ...
      0);
    context.getProject().setProjectIndicators(
      context.getProject().getIpProjectIndicators().stream().filter(c -> c.isActive()).collect(Collectors.toList()));
    // Get list of outcomes
    Set<IpElement> outcomesList = new HashSet<>();
    for (IpProjectIndicator ipProjectIndicator : context.getProject().getIpProjectIndicators().stream()
    ...
    for (IpElement outcome : outcomesList) {
      ...
      for (IpProjectIndicator ipProjectIndicator : context.getProject().getIpProjectIndicators().stream()
        .filter(i -> i.isActive() && i.getOutcomeId() == outcome.getId().intValue()
          && (i.getYear() == context.getSelectedYear() || i.getYear() == context.getSelectedYear() - 1
            || i.getYear() == context.getSelectedYear() + 1 || i.getYear() == APConstants.MID_OUTCOME_YEAR))
        .collect(Collectors.toList())) {
      ...
      for (IpIndicator ipIndicator : indicatorsList) {
        ...
        int ultimoProjectIndicator = (int) context.getProject().getIpProjectIndicators().stream()
          .filter(
        ...
        for (IpProjectIndicator ipProjectIndicator : context.getProject().getIpProjectIndicators().stream()
          ...
          targetCumulative = this.calculateAcumulativeTarget(ipProjectIndicator.getYear(), ipProjectIndicator, context);
          ...
          if (ipProjectIndicator.getYear() == context.getSelectedYear()) {
            isCurrent = true;
          }
          if (ultimoProjectIndicatorCount == ultimoProjectIndicator && ultimoIndicatorCount == ultimoIndicator) {
            showOutputs = true;
            List<IpElement> outputsList = this.getMidOutcomeOutputs(outcome.getId(), context);
  ...
  private String getDeliverableDataSharingFilePath(ReportingSummaryContext context) {
    String upload = apConfig.getDownloadURL();
    return upload + "/" + this.getDeliverableDataSharingFileRelativePath(context).replace('\\', '/');
  }

  private String getDeliverableDataSharingFileRelativePath(ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + context.getProject().getId()
      + File.separator + "deliverableDataSharing" + File.separator;
  }

  private TypedTableModel getDeliverablesReportingTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      ...
      new Class[] {Long.class, String.class, String.class, String.class, String.class, String.class, String.class,
        ...
        Boolean.class},
      0);
    if (!context.getProject().getDeliverables().isEmpty()) {
      // get Reporting deliverables
      List<Deliverable> deliverables = new ArrayList<>(context.getProject().getDeliverables().stream()
        .filter(d -> d.isActive() && d.getProject() != null && d.getProject().isActive()
          && d.getProject().getProjecInfoPhase(context.getSelectedPhase()).getReporting() != null
          && d.getProject().getGlobalUnitProjects().stream()
            .filter(gup -> gup.isActive() && gup.getGlobalUnit().getId().equals(context.getLoggedCrp().getId()))
            .collect(Collectors.toList()).size() > 0
          && d.getProject().getGlobalUnitProjects().stream()
            .filter(gup -> gup.isActive() && gup.getGlobalUnit().getId().equals(context.getLoggedCrp().getId()))
            .collect(Collectors.toList()).size() > 0
          && d.getDeliverableInfo(context.getSelectedPhase()).getStatus() != null
          && ((d.getDeliverableInfo(context.getSelectedPhase()).getStatus().intValue() == Integer
            .parseInt(ProjectStatusEnum.Complete.getStatusId())
            && (d.getDeliverableInfo(context.getSelectedPhase()).getYear() >= context.getSelectedYear()
              || (d.getDeliverableInfo(context.getSelectedPhase()).getNewExpectedYear() != null
                && d.getDeliverableInfo(context.getSelectedPhase()).getNewExpectedYear().intValue() >= context
                  .getSelectedYear())))
            || (d.getDeliverableInfo(context.getSelectedPhase()).getStatus().intValue() == Integer
      ...
      deliverables.sort((p1, p2) -> p1.getDeliverableInfo(context.getSelectedPhase())
        .isRequieriedReporting(context.getSelectedYear())
        .compareTo(p2.getDeliverableInfo(context.getSelectedPhase()).isRequieriedReporting(context.getSelectedYear())));
      ...
      for (Deliverable deliverable : deliverables) {
        ...
        String delivStatus =
          deliverable.getDeliverableInfo(context.getSelectedPhase()).getStatusName(context.getActualPhase());
        ...
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType() != null) {
          delivSubType = deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getName();
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 51
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 56
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 57
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 76
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 54
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 81
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 82
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 83
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 55
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 62
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 53
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 60
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 59
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 58
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 77
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 75
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 78
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 72
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 73) {
            showFAIR = true;
          }
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 51
            || deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getId() == 74) {
            showCompilance = true;
          }
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType()
            .getDeliverableCategory() != null) {
            delivType = deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType()
              .getDeliverableCategory().getName();
            // FAIR and deliverable publication
            if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getDeliverableType().getDeliverableCategory()
              .getId() == 49) {
              showFAIR = true;
        ...
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getYear() != 0) {
          delivYear = "" + deliverable.getDeliverableInfo(context.getSelectedPhase()).getYear();
        }
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrpClusterKeyOutput() != null) {
          keyOutput += "● ";
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrpClusterKeyOutput()
            .getCrpClusterOfActivity().getCrpProgram() != null) {
            keyOutput += deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrpClusterKeyOutput()
              .getCrpClusterOfActivity().getCrpProgram().getAcronym() + " - ";
          }
          keyOutput +=
            deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrpClusterKeyOutput().getKeyOutput();
        }
        ...
        for (DeliverableFundingSource dfs : deliverable.getDeliverableFundingSources().stream()
          .filter(d -> d.isActive() && d.getPhase() != null && d.getPhase().equals(context.getSelectedPhase()))
          .collect(Collectors.toList())) {
          fundingSources +=
            "● " + dfs.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getTitle() + "<br>";
        }
        ...
        String crossCutting = "";
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingNa() != null) {
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingNa() == true) {
            crossCutting += "&nbsp;&nbsp;&nbsp;&nbsp;● N/A <br>";
          }
        }
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingGender() != null) {
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingGender() == true) {
            crossCutting += "&nbsp;&nbsp;&nbsp;&nbsp;● Gender <br>";
          }
        }
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingYouth() != null) {
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingYouth() == true) {
            crossCutting += "&nbsp;&nbsp;&nbsp;&nbsp;● Youth <br>";
          }
        }
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingCapacity() != null) {
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingCapacity() == true) {
          ...
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getCrossCuttingGender() == true) {
            if (deliverable.getDeliverableGenderLevels() == null
              ...
              for (DeliverableGenderLevel dgl : deliverable.getDeliverableGenderLevels().stream()
                .filter(
                  dgl -> dgl.isActive() && dgl.getPhase() != null && dgl.getPhase().equals(context.getSelectedPhase()))
                .collect(Collectors.toList())) {
        ...
        if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getStatus() != null) {
          // Extended
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getStatus().intValue() == Integer
            .parseInt(ProjectStatusEnum.Extended.getStatusId())) {
            delivNewYear = deliverable.getDeliverableInfo(context.getSelectedPhase()).getNewExpectedYear();
            delivNewYearJustification =
              deliverable.getDeliverableInfo(context.getSelectedPhase()).getStatusDescription();
          }
          // Complete
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getStatus().intValue() == Integer
            .parseInt(ProjectStatusEnum.Complete.getStatusId())) {
        ...
        if (deliverable.getDeliverableDisseminations().stream().collect(Collectors.toList()).size() > 0
          ...
          if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getAdoptedLicense() != null) {
            if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getAdoptedLicense() == true) {
              delivLicense = deliverable.getDeliverableInfo(context.getSelectedPhase()).getLicense();
              if (delivLicense.equals("OTHER")) {
                delivLicense = deliverable.getDeliverableInfo(context.getSelectedPhase()).getOtherLicense();
                showDelivLicenseModifications = true;
                if (deliverable.getDeliverableInfo(context.getSelectedPhase()).getAllowModifications() != null
                  && deliverable.getDeliverableInfo(context.getSelectedPhase()).getAllowModifications() == true) {
        ...
        for (DeliverableDataSharingFile deliverableDataSharingFile : deliverable.getDeliverableDataSharingFiles()
          ...
          if (deliverableDataSharingFile.getFile() != null && deliverableDataSharingFile.getFile().isActive()) {
            dataSharing +=
              (this.getDeliverableDataSharingFilePath(context) + deliverableDataSharingFile.getFile().getFileName())
                .replace(" ", "%20") + "<br>";
          }
        }
            ...
            if (deliverableQualityCheck.getQualityAssurance().getId() == 2) {
              if (deliverableQualityCheck.getFileAssurance() != null
                ...
                qualityAssurance += "<br>● File: <font size=2 face='Segoe UI' color='blue'>"
                  + (this.getDeliverableUrl("Assurance", deliverable, context)
            ...
            if (deliverableQualityCheck.getDataDictionary().getId() == 2) {
              if (deliverableQualityCheck.getFileDictionary() != null
                ...
                dataDictionary += "<br>● File: <font size=2 face='Segoe UI' color='blue'>"
                  + (this.getDeliverableUrl("Dictionary", deliverable, context)
            ...
            if (deliverableQualityCheck.getDataTools().getId() == 2) {
              if (deliverableQualityCheck.getFileTools() != null && deliverableQualityCheck.getFileTools().isActive()) {
                tools += "<br>● File: <font size=2 face='Segoe UI' color='blue'>"
                  + (this.getDeliverableUrl("Tools", deliverable, context)
        ...
        String R = "";
        if (fairService.isR(deliverable.getId(), context.getActualPhase()) == null) {
          R += "#a3a3a3";
        } else {
          if (fairService.isR(deliverable.getId(), context.getActualPhase()) == true) {
            R += "#008000";
          } else {
            R += "#ca1010";
          }
        ...
        model.addRow(new Object[] {deliverable.getId(),
          deliverable.getDeliverableInfo(context.getSelectedPhase()).getTitle(), delivType, delivSubType, delivStatus,
  ...
  private TypedTableModel getDeliverablesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      new String[] {"deliverable_id", "title", "deliv_type", "deliv_sub_type", "deliv_status", "deliv_year",
      ...
      0);
    if (!context.getProject().getDeliverables().isEmpty()) {
      for (Deliverable deliverable : context.getProject().getDeliverables().stream()
        .sorted((d1, d2) -> Long.compare(d1.getId(), d2.getId()))
        .filter(d -> d.isActive() && d.getDeliverableInfo(context.getSelectedPhase()) != null
          && ((d.getDeliverableInfo().getStatus() == null
            && d.getDeliverableInfo().getYear() == context.getSelectedYear())
            || (d.getDeliverableInfo().getStatus() != null
              && d.getDeliverableInfo().getStatus().intValue() == Integer
                .parseInt(ProjectStatusEnum.Extended.getStatusId())
              && d.getDeliverableInfo().getNewExpectedYear() != null
              && d.getDeliverableInfo().getNewExpectedYear() == context.getSelectedYear())
            || (d.getDeliverableInfo().getStatus() != null
              && d.getDeliverableInfo().getYear() == context.getSelectedYear()
              && d.getDeliverableInfo().getStatus().intValue() == Integer
  ...
  public String getDeliverableUrl(String fileType, Deliverable deliverable, ReportingSummaryContext context) {
    return apConfig.getDownloadURL() + "/"
      + this.getDeliverableUrlPath(fileType, deliverable, context).replace('\\', '/');
  }

  public String getDeliverableUrlPath(String fileType, Deliverable deliverable, ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + deliverable.getId()
      + File.separator + "deliverable" + File.separator + fileType + File.separator;
  }

  private TypedTableModel getDescCoAsTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(new String[] {"description"}, new Class[] {String.class}, 0);
    if (context.getProject().getProjectClusterActivities() != null) {
      for (ProjectClusterActivity projectClusterActivity : context.getProject().getProjectClusterActivities().stream()
        .filter(c -> c.isActive() && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
  ...
  private TypedTableModel getDescTableModel(ProjectPartner projectLeader, Boolean hasRegions,
    ...
    String title = context.getProjectInfo().getTitle();
    String startDate = null;
    String endDate = null;
    if (context.getProjectInfo().getStartDate() != null) {
      startDate = formatter.format(context.getProjectInfo().getStartDate());
    }
    if (context.getProjectInfo().getEndDate() != null) {
      endDate = formatter.format(context.getProjectInfo().getEndDate());
    }
    if (context.getProjectInfo().getLiaisonInstitution() != null) {
      ml = context.getProjectInfo().getLiaisonInstitution().getAcronym();
    }
    if (context.getProjectInfo().getLiaisonUser() != null) {
      ml = context.getProjectInfo().getLiaisonUser().getLiaisonInstitution().getAcronym();
      mlContact = context.getProjectInfo().getLiaisonUser().getComposedName() + "\n&lt;"
        + context.getProjectInfo().getLiaisonUser().getUser().getEmail() + "&gt;";
      if (context.getProjectInfo().getLiaisonUser() != null) {
        mlContact = context.getProjectInfo().getLiaisonUser().getComposedName() + "\n&lt;"
          + context.getProjectInfo().getLiaisonUser().getUser().getEmail() + "&gt;";
      }
      ...
      for (ProjectBudget projectBudget : context.getProject().getProjectBudgets()
        .stream().filter(pb -> pb.isActive() && pb.getYear() == context.getSelectedYear()
          && pb.getFundingSource() != null && pb.getPhase() != null && pb.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
        typeList.add(
          projectBudget.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getBudgetType().getName());
      }
      // Remove duplicates
      ...
      String status = ProjectStatusEnum.getValue(context.getProjectInfo().getStatus().intValue()).getStatus();
      ...
      if (context.getProject().getLeaderPerson(context.getSelectedPhase()) != null
        && context.getProject().getLeaderPerson(context.getSelectedPhase()).getUser() != null) {
        leader = context.getProject().getLeaderPerson(context.getSelectedPhase()).getUser().getComposedName() + "\n&lt;"
          + context.getProject().getLeaderPerson(context.getSelectedPhase()).getUser().getEmail() + "&gt;";
      }
      String summary = context.getProjectInfo().getSummary();
      if (summary != null) {
        if (summary.equals("")) {
          summary = null;
        }
      }
      String analysis = context.getProjectInfo().getGenderAnalysis();
      if (analysis != null) {
        if (analysis.equals("")) {
          analysis = null;
        }
      ...
      String crossCutting = "";
      if (context.getProjectInfo().getCrossCuttingNa() != null) {
        if (context.getProjectInfo().getCrossCuttingNa() == true) {
          crossCutting += "● N/A <br>";
        }
      }
      if (context.getProjectInfo().getCrossCuttingGender() != null) {
        if (context.getProjectInfo().getCrossCuttingGender() == true) {
          crossCutting += "● Gender <br>";
        }
      }
      if (context.getProjectInfo().getCrossCuttingYouth() != null) {
        if (context.getProjectInfo().getCrossCuttingYouth() == true) {
          crossCutting += "● Youth <br>";
        }
      }
      if (context.getProjectInfo().getCrossCuttingCapacity() != null) {
        if (context.getProjectInfo().getCrossCuttingCapacity() == true) {
          crossCutting += "● Capacity Development <br>";
        }
      }
      if (context.getProjectInfo().getCrossCuttingGender() != null) {
        if (context.getProjectInfo().getCrossCuttingGender() == false) {
          if (context.getProjectInfo().getDimension() == null || context.getProjectInfo().getDimension().isEmpty()) {
            ...
            crossCutting += "<br><br>" + "<b>Reason for not addressing gender dimension: </b>"
              + context.getProjectInfo().getDimension();
          }
        }
      }
      if (crossCutting.isEmpty()) {
      ...
      model.addRow(new Object[] {title, startDate, endDate, ml, mlContact, type, status, orgLeader, leader, summary,
        context.getSelectedCycle(), analysis, crossCutting, hasRegions, mlText, mlContactText});
    }
    return model;
  }

  private TypedTableModel getExpectedStudiesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      ...
      new Class[] {String.class, String.class, String.class, String.class, String.class, String.class, String.class},
      0);
    if (!context.getProject().getProjectExpectedStudies().isEmpty()) {
      for (ProjectExpectedStudy projectExpectedStudy : context.getProject().getProjectExpectedStudies().stream()
        .filter(c -> c.isActive() && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
  ...
  private IpIndicator getFinalIndicator(IpIndicator ipIndicator) {
  ...
  public void getFooterSubreports(HashMap<String, Element> hm, ReportFooter reportFooter) {
  ...
  public String getHighlightsImagesUrl(ReportingSummaryContext context) {
    return apConfig.getDownloadURL() + "/" + this.getHighlightsImagesUrlPath(context).replace('\\', '/');
  }

  public String getHighlightsImagesUrlPath(long projectID, ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + context.getProject().getId()
      + File.separator + "hightlightsImage" + File.separator;
  }


  public String getHighlightsImagesUrlPath(ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + context.getProject().getId()
      + File.separator + "hightlightsImage" + File.separator;
  }

  private String getHightlightImagePath(long projectID, ReportingSummaryContext context) {
    return apConfig.getUploadsBaseFolder() + File.separator + this.getHighlightsImagesUrlPath(projectID, context)
      + File.separator;
  }

  private TypedTableModel getLeveragesTableModel(ReportingSummaryContext context) {
    // Decimal format
    DecimalFormat myFormatter = new DecimalFormat("###,###.00");
    ...
    for (ProjectLeverage projectLeverage : context.getProject().getProjectLeverages().stream()
      .filter(pl -> pl.isActive() && pl.getYear() == context.getSelectedYear()).collect(Collectors.toList())) {
  ...
  private TypedTableModel getLocationsTableModel(ReportingSummaryContext context) {
    ...
    if (!context.getProject().getProjectLocations().isEmpty()) {
      // Get all selected and show it
      List<LocElement> locElementsAll = locElementManager.findAll();
      for (ProjectLocationElementType projectLocType : context.getProject().getProjectLocationElementTypes().stream()
      ...
      for (ProjectLocation pl : context.getProject().getProjectLocations().stream()
        .filter(c -> c.isActive() && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
  ...
  private TypedTableModel getMasterTableModel(List<CrpProgram> flagships, List<CrpProgram> regions,
    ProjectPartner projectLeader, ReportingSummaryContext context) {
    ...
    if (context.getProjectInfo().getAdministrative() == false) {
      if (flagships != null) {
      ...
      if (context.getProjectInfo().getNoRegional() != null && context.getProjectInfo().getNoRegional()) {
        title += "Global" + "-";
      } else {
    ...
    title += "P" + Long.toString(context.getProject().getId());
    // Get datetime
    ...
    for (Submission submission : context.getProject().getSubmissions().stream()
      .filter(c -> c.getCycle().equals(context.getSelectedCycle()) && c.getYear() == context.getSelectedYear()
    ...
    if (!submissions.isEmpty()) {
      ...
      if (!context.getSelectedCycle().isEmpty() && context.getSelectedYear() != 0) {
        if (context.getSelectedCycle().equals("Reporting")) {
          submission = "Submission for " + context.getSelectedCycle() + " cycle " + context.getSelectedYear()
            + ": &lt;not submited&gt;";
        } else {
          submission = "Submission for " + context.getSelectedCycle() + " cycle " + context.getSelectedYear()
            + ": &lt;pending&gt;";
    ...
    GlobalUnitProject globalUnitProject = context.getProject().getGlobalUnitProjects().stream()
    ...
    if (context.getProjectInfo().getAdministrative() != null) {
      if (context.getProjectInfo().getAdministrative() == true) {
        type = "Management Project";
      }
      isAdministrative = context.getProjectInfo().getAdministrative();
    } else {
      isAdministrative = false;
    }
    Boolean isNew = context.isProjectNew();
    Boolean hasGender = false;
    try {
      hasGender = context.isBudgetGenderSpecifityEnabled();
    } catch (Exception e) {
    ...
    if (context.getTargetUnitList().size() > 0) {
      hasTargetUnit = true;
    }
    ...
    Long phaseID = context.getSelectedPhase().getId();

    model.addRow(new Object[] {title, centerURL, currentDate, submission, context.getSelectedCycle(), isNew,
      isAdministrative, type, context.getProjectInfo().getLocationGlobal(), context.isPhaseOne(), hasGender,
      hasTargetUnit, context.getHasW1W2Co(), hasActivities, phaseID});
    return model;
  }

  private List<IpElement> getMidOutcomeOutputs(long midOutcomeID, ReportingSummaryContext context) {
    List<IpProjectContribution> ipProjectContributions =
      context.getProject().getIpProjectContributions().stream().filter(c -> c.isActive()).collect(Collectors.toList());
    context.getProject().setMogs(new ArrayList<>());
    for (IpProjectContribution ipProjectContribution : ipProjectContributions) {
      context.getProject().getMogs().add(ipProjectContribution.getIpElementByMogId());
    }
    ...
    for (IpElement ipElement : elements) {
      if (!this.containsOutput(ipElement.getId(), midOutcomeID, context)) {
  ...
  private TypedTableModel getOtherContributionsCrpsTableModel(ReportingSummaryContext context) {
    ...
    for (ProjectCrpContribution projectCrpContribution : context.getProject().getProjectCrpContributions().stream()
  ...
  private TypedTableModel getOtherContributionsDetailTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
    ...
    for (OtherContribution otherContribution : context.getProject().getOtherContributions().stream()
      .filter(oc -> oc.isActive()).collect(Collectors.toList())) {
      ...
      otherContributionyear = context.getSelectedYear();
      model
  ...
  private TypedTableModel getOtherContributionsTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(new String[] {"contribution"}, new Class[] {String.class}, 0);
    String contribution = null;
    for (ProjectOtherContribution projectOtherContribution : context.getProject().getProjectOtherContributions()
      .stream().filter(poc -> poc.isActive()).collect(Collectors.toList())) {
  ...
  private TypedTableModel getOutcomesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      ...
      0);
    if (!context.getProject().getProjectOutcomes().isEmpty()) {
      for (ProjectOutcome projectOutcome : context.getProject().getProjectOutcomes().stream()
        .filter(c -> c.isActive() && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
        ...
        if (projectOutcome.getCrpProgramOutcome() != null) {
          ...
          if (context.isIpOutcomeIndicatorSpecificityEnabled()) {
  ...
  private TypedTableModel getOverviewByMogsTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
    ...
    for (IpProjectContributionOverview ipProjectContributionOverview : context.getProject()
      .getIpProjectContributionOverviews().stream().sorted((co1, co2) -> co2.getYear() - co1.getYear())
      .filter(co -> co.isActive()).collect(Collectors.toList())) {
  ...
  private TypedTableModel getPartnersLessonsTableModel(ReportingSummaryContext context) {
    TypedTableModel model =
      new TypedTableModel(new String[] {"year", "lesson"}, new Class[] {Integer.class, String.class}, 0);
    if (!context.getSelectedCycle().equals("")) {
      for (ProjectComponentLesson pcl : context.getProject().getProjectComponentLessons().stream()
        .sorted((p1, p2) -> p1.getYear() - p2.getYear())
        .filter(c -> c.isActive() && c.getComponentName().equals("partners")
          && c.getCycle().equals(context.getSelectedCycle()) && c.getYear() == context.getSelectedYear()
          && c.getPhase() != null && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
        String lessons = null;
  ...
  private TypedTableModel getPartnersOtherTableModel(ProjectPartner projectLeader, ReportingSummaryContext context) {
    TypedTableModel model =
    ...
    if (projectLeader.getId() != null) {
      ...
      for (ProjectPartner projectPartner : context.getProject().getProjectPartners().stream()
        .filter(c -> c.isActive() && c.getId() != projectLeader.getId() && c.getPhase() != null
          && c.getPhase().equals(context.getSelectedPhase()))
        .collect(Collectors.toList())) {
        countryOffices = null;
  ...
  private TypedTableModel getPartnersTableModel(ReportingSummaryContext context) {
    TypedTableModel model =
      new TypedTableModel(new String[] {"count", "overall"}, new Class[] {Integer.class, String.class}, 0);
    int partnersSize = 0;
    List<ProjectPartner> projectPartners = context.getProject().getProjectPartners().stream()
      .filter(pp -> pp.isActive() && pp.getPhase() != null && pp.getPhase().equals(context.getSelectedPhase()))
      .collect(Collectors.toList());
    ...
    if (context.getSelectedCycle().equals("Reporting")) {
      ...
      for (ProjectPartner projectPartner : context.getProject().getProjectPartners().stream()
        ...
        for (ProjectPartnerOverall projectPartnerOverall : projectPartner.getProjectPartnerOveralls().stream()
          .filter(ppo -> ppo.getYear() == context.getSelectedYear()).collect(Collectors.toList())) {
  ...
  private TypedTableModel getProjectHighlightReportingTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
      ...
      0);
    SimpleDateFormat formatter = new SimpleDateFormat("MMM yyyy");
    for (ProjectHighlight projectHighlight : context.getProject().getProjectHighligths().stream()
      .filter(ph -> ph.isActive() && ph.getYear() != null && ph.getYear() >= context.getSelectedYear())
      .collect(Collectors.toList())) {
      ...
      if (projectHighlight.getFile() != null) {
        ...
        image = this.getHightlightImagePath(projectHighlight.getProject().getId(), context)
  ...
  private TypedTableModel getProjectOtherOutcomesTableModel(ReportingSummaryContext context) {
    TypedTableModel model =
      new TypedTableModel(new String[] {"out_statement", "year"}, new Class[] {String.class, Integer.class}, 0);
    String outStatement = "";
    int outcomeYear = 0;
    for (ProjectOutcomePandr projectOutcomePandr : context.getProject().getProjectOutcomesPandr().stream()
      .sorted((p1, p2) -> p1.getYear() - p2.getYear())
      .filter(pop -> pop.isActive() && pop.getYear() != 2019 && pop.getYear() != context.getSelectedYear())
      .collect(Collectors.toList())) {
  ...
  private TypedTableModel getProjectOutcomesTableModel(ReportingSummaryContext context) {
    TypedTableModel model = new TypedTableModel(
    ...
    for (ProjectOutcomePandr projectOutcomePandr : context.getProject().getProjectOutcomesPandr().stream()
      .filter(pop -> pop.isActive()
        && (pop.getYear() == APConstants.MID_OUTCOME_YEAR || pop.getYear() == context.getSelectedYear()))
      .collect(Collectors.toList())) {
      ...
      if (projectOutcomePandr.getYear() == context.getSelectedYear()) {
        if (projectOutcomePandr.getStatement() != null) {
          if (!projectOutcomePandr.getStatement().isEmpty()) {
        ...
        if (projectOutcomePandr.getFile() != null) {
          file = (this.getProjectOutcomeUrl(context) + projectOutcomePandr.getFile().getFileName()).replace(" ", "%20");
        }
      }
    }
    if (!context.getSelectedCycle().equals("")) {
      for (ProjectComponentLesson pcl : context.getProject().getProjectComponentLessons().stream()
        ...
        .filter(pcl -> pcl.isActive() && pcl.getComponentName().equals("outcomesPandR")
          && pcl.getPhase().equals(context.getSelectedPhase()) && pcl.getCycle().equals(context.getSelectedCycle())
          && pcl.getYear() == context.getSelectedYear())
        .collect(Collectors.toList())) {
    ...
    model.addRow(new Object[] {outStatement, outStatementCurrent, outProgressCurrent, communicationCurrent,
      context.getSelectedYear(), lessons, file});
    return model;
  }

  private String getProjectOutcomeUrl(ReportingSummaryContext context) {
    return apConfig.getDownloadURL() + "/" + this.getProjectOutcomeUrlPath(context).replace('\\', '/');
  }

  private String getProjectOutcomeUrlPath(ReportingSummaryContext context) {
    return apConfig.getProjectsBaseFolder(context.getCrpAcronym()) + File.separator + context.getProject().getId()
      + File.separator + "project_outcome" + File.separator;
  }

  public byte[] getReport(ReportingSummaryContext context) throws Exception {
    ...
    try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
      ...
      if (context.getSelectedCycle().equals("Planning")) {
      ...
      if (context.getProject() != null) {
        ...
        // get project leader
        ProjectPartner projectLeader = context.getProject().getLeader(context.getSelectedPhase());
        // get Flagships related to the project sorted by acronym
        List<CrpProgram> flagships = new ArrayList<>();
        for (ProjectFocus projectFocuses : context.getProject().getProjectFocuses().stream()
        ...
        if (context.isHasProgramRegions() != false) {
          for (ProjectFocus projectFocuses : context.getProject().getProjectFocuses().stream()
        ...
        TypedTableModel model = this.getMasterTableModel(flagships, regions, projectLeader, context);
        sdf.addTable(masterQueryName, model);
        masterReport.setDataFactory(cdf);
        // Set i8n for pentaho
        masterReport = this.addi8nParameters(masterReport, context);
        // Set columns parameters (x and width)
        masterReport = this.addColumnParameters(masterReport, context);

        // Start Setting Planning Subreports
        // Subreport Description
        args.add(projectLeader);
        args.add(context.isHasProgramRegions());
        this.fillSubreport((SubReport) hm.get("description"), "description", args, context);
        // Description Flagships
        args.clear();
        args.add(flagships);
        this.fillSubreport((SubReport) hm.get("Flagships"), "description_flagships", args, context);
        // Description Regions
        args.clear();
        args.add(regions);
        this.fillSubreport((SubReport) hm.get("Regions"), "description_regions", args, context);
        if (context.getSelectedCycle().equals("Planning")) {
          // Description CoAs
          args.clear();
          this.fillSubreport((SubReport) hm.get("Description_CoAs"), "description_coas", args, context);
        }

        // Subreport Partners
        this.fillSubreport((SubReport) hm.get("partners"), "partners_count", args, context);
        // Subreport Partner Leader
        args.clear();
        args.add(projectLeader);
        this.fillSubreport((SubReport) hm.get("partner_leader"), "institution_leader", args, context);
        // Subreport Partner Others
        args.clear();
        args.add(projectLeader);
        this.fillSubreport((SubReport) hm.get("partners_others"), "partners_others_ins", args, context);
        // Note: Contacts for partners are filled by queries inside the prpt
        // Subreport Partner Lessons
        args.clear();
        this.fillSubreport((SubReport) hm.get("partner_lessons"), "partner_lessons", args, context);

        // Subreport Locations
        args.clear();
        this.fillSubreport((SubReport) hm.get("locations"), "locations", args, context);

        if (context.getSelectedCycle().equals("Planning")) {
          // Subreport Outcomes
          args.clear();
          this.fillSubreport((SubReport) hm.get("outcomes"), "outcomes_list", args, context);
          this.fillSubreport((SubReport) hm.get("expected_studies"), "expected_studies", args, context);
        } else {
          // Subreport Project Outcomes
          args.clear();
          this.fillSubreport((SubReport) hm.get("project_outcomes"), "project_outcomes", args, context);
          this.fillSubreport((SubReport) hm.get("other_outcomes"), "other_outcomes", args, context);
          this.fillSubreport((SubReport) hm.get("ccafs_outcomes"), "ccafs_outcomes", args, context);
          this.fillSubreport((SubReport) hm.get("other_contributions"), "other_contributions", args, context);
          this.fillSubreport((SubReport) hm.get("other_contributions_detail"), "other_contributions_detail", args,
            context);
          this.fillSubreport((SubReport) hm.get("other_contributions_crps"), "other_contributions_crps", args, context);
          this.fillSubreport((SubReport) hm.get("case_studies"), "case_studies", args, context);
          this.fillSubreport((SubReport) hm.get("overview_by_mogs"), "overview_by_mogs", args, context);
        }
        ...
        if (context.getSelectedCycle().equals("Planning")) {
          // Subreport Outcomes
          args.clear();
          this.fillSubreport((SubReport) hm.get("deliverables"), "deliverables_list", args, context);
        } else {
          // args.clear();
          this.fillSubreport((SubReport) hm.get("deliverables"), "deliverables_list_reporting", args, context);
          this.fillSubreport((SubReport) hm.get("project_highlight"), "project_highlight", args, context);
        }

        // Subreport Activities
        args.clear();
        if (context.getSelectedCycle().equals("Planning")) {
          this.fillSubreport((SubReport) hm.get("activities"), "activities_list", args, context);
        } else {
        ...
        if (context.getSelectedCycle().equals("Planning")) {
          ...
          args.clear();
          this.fillSubreport((SubReport) hm.get("budgets"), "budget_summary", args, context);
          // Subreport BudgetsbyPartners
          this.fillSubreport((SubReport) hm.get("budgets_by_partners"), "budgets_by_partners_list", args, context);
          // Subreport BudgetsbyCoas
          this.fillSubreport((SubReport) hm.get("budgets_by_coas"), "budgets_by_coas_list", args, context);
        } else {
          // Subreport Leverages for reporting
          this.fillSubreport((SubReport) hm.get("leverages"), "leverages", args, context);
        }
  ...
  private TypedTableModel getRLTableModel(List<CrpProgram> regions, ReportingSummaryContext context) {
   ...
   */
  private String getTotalAmount(long institutionId, int year, long budgetType, Long projectId, Integer coFinancing,
    ReportingSummaryContext context) {
    return projectBudgetManager.amountByBudgetType(institutionId, year, budgetType, projectId, coFinancing,
      context.getSelectedPhase().getId());
  }
   ...
   * @return
   */
  private double getTotalGender(long institutionId, int year, long budgetType, long projectID, Integer coFinancing,
    ReportingSummaryContext context) {

    List<ProjectBudget> budgets = projectBudgetManager.getByParameters(institutionId, year, budgetType, projectID,
      coFinancing, context.getSelectedPhase().getId());

    double totalGender = 0;
    if (budgets != null) {
      for (ProjectBudget projectBudget : budgets) {
        if (projectBudget.getPhase().equals(context.getSelectedPhase())) {
   ...
   * @return
   */
  private double getTotalGender(long institutionId, int year, long budgetType, Project project, Integer coFinancing,
    ReportingSummaryContext context) {
   ...
   * @return
   */
  private double getTotalGenderPer(long institutionId, int year, long budgetType, long projectId, Integer coFinancing,
    ReportingSummaryContext context) {

    String totalAmount = this.getTotalAmount(institutionId, year, budgetType, projectId, coFinancing, context);

    double dTotalAmount = Double.parseDouble(totalAmount);

    double totalGender = this.getTotalGender(institutionId, year, budgetType, projectId, coFinancing, context);
   ...
   */
  private double getTotalYear(int year, long type, Project project, Integer coFinancing,
    ReportingSummaryContext context) {
    double total = 0;
      ...
      case 1:
        for (ProjectBudget pb : project.getProjectBudgets().stream()
          .filter(pb -> pb.isActive() && pb.getYear() == year && pb.getBudgetType() != null
            ...
            && pb.getPhase().equals(context.getSelectedPhase()))
          .collect(Collectors.toList())) {
      ...
      case 2:
        for (ProjectBudget pb : project.getProjectBudgets().stream()
          .filter(pb -> pb.isActive() && pb.getYear() == year && pb.getBudgetType() != null
            ...
            && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2() != null
            && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2().booleanValue() == true
            && pb.getPhase() != null && pb.getPhase().equals(context.getSelectedPhase()))
          .collect(Collectors.toList())) {
          total = total + pb.getAmount();
        }
      ...
      case 3:
        for (ProjectBudget pb : project.getProjectBudgets().stream()
          .filter(pb -> pb.isActive() && pb.getYear() == year && pb.getBudgetType() != null
            && pb.getBudgetType().getId() == type && pb.getFundingSource() != null
            && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2() != null
            && pb.getFundingSource().getFundingSourceInfo(context.getSelectedPhase()).getW1w2().booleanValue() == false
            && pb.getPhase() != null && pb.getPhase().equals(context.getSelectedPhase()))
          .collect(Collectors.toList())) {
   ...
   */
  private boolean isPPA(Institution institution, ReportingSummaryContext context) {
    if (institution == null) {
      return false;
    }
    if (institution.getId() != null) {
      ...
      if (institution != null) {
        if (institution.getCrpPpaPartners().stream()
          .filter(c -> c.isActive() && c.getPhase().equals(context.getActualPhase())).collect(Collectors.toList())
          .size() > 0) {
          return true;
        }
      }
  ...
  private boolean isRegionalOutcome(IpElement outcome) {
    List<IpElement> translatedOf =
